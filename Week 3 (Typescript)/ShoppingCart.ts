...
class ShoppingCart{
    ...
    removeProductByName(name: string): void{
        ...
        const index = this.products.findIndex((product) => 
            {return product.getName().toLowerCase() === name});  //-1 is returned if name is not found in products array. If none of the products is == to that name you want to delete
        if(index !== -1){
